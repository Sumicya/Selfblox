...
local function nameHint(name, hints)
	...
	for _, h in ipairs(hints) do
		if n:find(h, 1, true) then return h end
	end
	return nil
end
...
local function inspectModel(model)
	...
	pcall(function()
		...
		for _, d in ipairs(model:GetDescendants()) do
			n += 1
			if n > CFG.MaxModelParts then break end
			local cn = d.ClassName
			if cn == "Seat" or cn == "VehicleSeat" then
				info.hasSeat = true
				if #info.seatNames < 4 then info.seatNames[#info.seatNames + 1] = d.Name end
			...
			elseif cn == "RemoteEvent" or cn == "RemoteFunction" or cn == "UnreliableRemoteEvent" then
				if #info.remotes < 8 then info.remotes[#info.remotes + 1] = d end
			elseif cn == "ClickDetector" or cn == "ProximityPrompt" then
				if #info.clickables < 8 then info.clickables[#info.clickables + 1] = d end
			elseif d:IsA("BasePart") and cn ~= "Terrain" then
				info.partCount += 1
				if d.Anchored then info.anchoredCount += 1 end
				if nameHint(d.Name, WING_HINTS) then info.wingCount += 1 end
				local vol = d.Size.X * d.Size.Y * d.Size.Z
				if vol > bestVol then bestVol, info.mainPart = vol, d end
		...
		if info.mainPart then info.bodyColor = info.mainPart.Color end
	end)
	pcall(function()
		local list = {}
	...
	local score = 0
	if info.hint then score += 40 end
	if info.hasSeat then score += 35 end
	if info.wingCount >= 1 then score += 15 end
	if info.wingCount >= 2 then score += 10 end
	if info.partCount >= 8 then score += 10 end
	if info.partCount > 0 and info.anchoredCount < info.partCount then score += 5 end
	if info.occupant then score += 5 end
	info.score = score
	return info
end
	...
	if info.bodyColor then
		...
		pcall(function()
			for _, t in ipairs(Teams:GetTeams()) do
				...
				if d < bestD then bestTeam, bestD = t, d end
			end
		end)
...
local function teamInfoFor(info)
	...
	local now = os.clock()
	if rec and now - rec.at < CFG.TeamInfoTTL then return rec.info end
...
local function dumpPlaneInfo(log, info)
	local tags = {}
	if info.isMine then tags[#tags + 1] = "★我乘坐" end
	if info.occupant then tags[#tags + 1] = "乘员:" .. info.occupant.Name end
	...
	if info.dist then log(string.format("    距离: %.0f", info.dist)) end
	if #info.attributes > 0 then log("    属性: " .. table.concat(info.attributes, "; ")) end
	if #info.values > 0 then log("    Values: " .. table.concat(info.values, "; ")) end
...
local function scanPlanes()
	...
	local function visit(container, depth)
		if depth > 4 or inspected > 300 or #results >= 80 then return end
		for _, child in ipairs(container:GetChildren()) do
			if inspected > 300 or #results >= 80 then return end
	...
	table.sort(results, function(a, b)
		if a.isMine ~= b.isMine then return a.isMine end
...
local function scanRemotes()
	...
	pcall(function()
		...
		for _, d in ipairs(workspace:GetDescendants()) do
			n += 1
			if n > 3000 or #results >= 500 then break end
			local cn = d.ClassName
			if cn == "RemoteEvent" or cn == "RemoteFunction" or cn == "UnreliableRemoteEvent" then
				results[#results + 1] = {remote = d, hint = nameHint(d.Name, REMOTE_HINTS)}
			end
...
local function spyFilter(path)
	if spyLogAll then return true end
...
local function stopSpy()
	if not namecallHooked then return end
...
local function buildReport()
	...
	do
		...
		if #pattrs > 0 then log("玩家属性: " .. table.concat(pattrs, "; ")) end
	end
	...
	pcall(function()
		...
		if char then
			for _, item in ipairs(char:GetChildren()) do
				if item:IsA("Tool") then log("  [手持] Tool '" .. item.Name .. "'") end
			end
		end
	end)
	...
	for _, rec in ipairs(remoteResults) do
		if rec.hint then hinted[#hinted + 1] = rec else plain[#plain + 1] = rec end
	end
	log("-- 名字带关键词 --")
	...
	if #bulletResults == 0 then log("  (未抓到 → 子弹可能不叫这些名字，或即生即灭)") end
	...
	if #spyLog == 0 then log("  (空 → 打开「发侦听」后进飞机开几枪，再点「写文件」)") end
	for _, entry in ipairs(spyLog) do log("  " .. entry) end

	log("")
	log(string.format("== OnClientEvent 记录 %d 条 ==", #incomingLog))
	if #incomingLog == 0 then log("  (空 → 打开「收侦听」)") end
	for _, entry in ipairs(incomingLog) do log("  " .. entry) end
...
local function refreshStatus()
	...
	for _, info in ipairs(planeResults) do
		...
		if ti and ti.playerTeam and myTeam then
			if ti.playerTeam == myTeam then friendly += 1 else enemy += 1 end
		else
			unknown += 1
		end
	end
...
local function applyHighlights()
	...
	local n = 0
	for _, info in ipairs(planeResults) do
		if n >= CFG.HighlightCap then break end
		local model = info.model
...
local function setToggle(btn, on, label)
	...
	btn.BackgroundColor3 = on and CFG.Col.On or CFG.Col.Off
end
...
-- ===== 上下文 / 循环 =====
local function syncSeatContext()
...
M.debug(function()
	...
	log("writefile: " .. tostring(HAS_WRITEFILE))
	log("setclipboard: " .. tostring(type(setclipboard) == "function"))
	log("hookmetamethod: " .. tostring(type(hookmetamethod) == "function"))
	log("getnamecallmethod: " .. tostring(type(getnamecallmethod) == "function"))
	log("newcclosure: " .. tostring(type(newcclosure) == "function"))
...
M.done(function()
	...
	for _, c in pairs(watchConns) do pcall(function() c:Disconnect() end) end
	table.clear(watchConns)
	for _, h in pairs(highlightMap) do pcall(function() h:Destroy() end) end
